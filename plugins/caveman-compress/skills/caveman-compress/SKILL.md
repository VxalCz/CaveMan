...
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts compress CLAUDE.md

# Batch: directory or glob
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts compress docs/
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts compress "*.md"

# Skip files where savings would be < 30%
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts compress CLAUDE.md --min-savings 30

# Audit (no writes)
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts audit
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts audit . --min-savings 20

# Undo
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts undo CLAUDE.md

# Stats
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts stats

# Diff
cd "${CLAUDE_PLUGIN_ROOT}" && python -m scripts diff CLAUDE.md
```
